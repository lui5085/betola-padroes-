...
export class OddsUpdateService {
  ...
  // Run every 2 hours during match hours (10:00 - 23:00 Brazil time) to conserve API quota
  @Cron('0 */2 * * *')
  async updateOdds() {
  ...
  // Run every 10 minutes for live matches to conserve API quota
  @Cron('*/10 * * * *')
  async updateLiveOdds() {
    ...
    try {
      ...
      const leagueId = leagueResponse.id;
      
      // Get live matches
      const liveMatches = await this.footballApiService.getLiveMatches(leagueId);
      
      if (liveMatches.matches.length > 0) {
        this.logger.log(`Found ${liveMatches.matches.length} live matches`);
        
        for (const fixture of liveMatches.matches) {
          try {
            // Find match in our database
            const match = await this.matchesRepository.findByExternalId(fixture.id.toString());
            ...
            if (match) {
              ...
              if (fixture.score?.fullTime?.home !== null && fixture.score?.fullTime?.away !== null) {
                match.updateScore(fixture.score.fullTime.home, fixture.score.fullTime.away);
                await this.matchesRepository.update(match);
              }
            }
          } catch (error) {
            this.logger.error(`Failed to update live match ${fixture.id}`, error);
          }
        }
      }
    } catch (error) {
  ...
  // Run every 6 hours to check for new matches to conserve API quota  
  @Cron('0 */6 * * *')
