...
import { Market } from '@betola/core/modules/betting/domain/entities/market';
...
export class SyncMatchOddsUseCase {
  ...
  private async syncOddsForMatch(
    ...
    try {
      ...
      const bookmakerIds = [8, 6, 1, 3]; // Bet365, Bwin, Unibet, William Hill
      let bookmaker = null;
      
      for (const id of bookmakerIds) {
        bookmaker = oddsData.bookmakers.find(b => b.id === id);
        if (bookmaker) break;
      }
      
      if (!bookmaker || !bookmaker.bets.length) {
        return { created: 0, updated: 0, error: 'No supported bookmaker odds found' };
      }
  ...
  private mapBetNameToMarketType(betName: string): MarketType | null {
    const mappings: Record<string, MarketType> = {
      ...
      'Goals Over/Under': MarketType.OVER_UNDER_GOALS,
      'Asian Handicap': MarketType.ASIAN_HANDICAP,
      'Correct Score': MarketType.CORRECT_SCORE,
      'First Half Winner': MarketType.FIRST_HALF_RESULT,
      'Double Chance': MarketType.DOUBLE_CHANCE,
      'Odd/Even': MarketType.ODD_EVEN_GOALS,
      'Total Goals': MarketType.TOTAL_GOALS
    };

    // Try exact match first
    if (mappings[betName]) {
      return mappings[betName];
    }

    // Try partial matches
    const betNameLower = betName.toLowerCase();
    for (const [key, type] of Object.entries(mappings)) {
      if (betNameLower.includes(key.toLowerCase())) {
        return type;
      }
    }

    // Special handling for over/under with different lines
    if (betNameLower.includes('over/under')) {
  ...
  private normalizeOptionName(value: string, marketType: MarketType): string {
    switch (marketType) {
      ...
      case MarketType.ODD_EVEN_GOALS:
        if (value.toLowerCase() === 'odd') return 'Odd';
        if (value.toLowerCase() === 'even') return 'Even';
        break;
    }
  ...
  private getMarketDisplayName(betName: string, marketType: MarketType): string {
    const displayNames: Record<MarketType, string> = {
      ...
      [MarketType.TOTAL_GOALS]: 'Total de Gols Exato',
