// packages/core/shared/types/token.spec.ts

import { Token, InvalidTokenError } from './token';
...
describe('Token', () => {
  ...
  it('deve lançar erro para Token inválido (tamanho errado)', () => {
    ...
    expect(() => new Token(invalidToken)).toThrow(InvalidTokenError);
  });

  it('deve lançar erro para Token inválido (caracteres não hex)', () => {
    const invalidToken = 'g'.repeat(64); // 'g' não é hexadecimal
    expect(() => new Token(invalidToken)).toThrow(InvalidTokenError);
  });

  it('deve comparar igualdade corretamente', () => {
    const token = 'b'.repeat(64);
    const t1 = new Token(token);
    const t2 = new Token(token);
  ...
  it('deve diferenciar Tokens diferentes', () => {
    ...
    const t1 = new Token(token1);
    const t2 = new Token(token2);
